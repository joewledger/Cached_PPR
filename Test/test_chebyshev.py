...
def test_chebyshev_top_k():
    ...
    print(ppr.get_l1_norm(final_vector, top_k_result.final_vector))


def test_indexed_chebyshev_top_k():
    weight_matrix = io_utils.load_csr_matrix("Data/Email-Enron.mat")
    dimension = weight_matrix.shape[0]
    query_nodes = random.sample(range(dimension),5)
    alpha = .5
    k = 10
    ...
    indexed_top_k_result = ppr.chebyshev_top_k(weight_matrix, start_vector, restart_vector, alpha, k, eps=1E-10)
